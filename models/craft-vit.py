"""
CraftViT — Text-to-Image Transformer
=====================================
Encoder-decoder transformer for autoregressive pixel generation from text prompts.
Built from scratch following the same patterns as llm-from-scratch:
  - Custom LayerNorm, GELU, FeedForward
  - Manual MultiHeadAttention (both self and cross)
  - Config-dict driven architecture
  - nn.Embedding for positional encoding (learned)
"""
...
# ---------------------------------------------------------------------------
# Encoder Block  (bidirectional self-attention — no causal mask)
# ---------------------------------------------------------------------------
...
class EncoderBlock(nn.Module):
    def __init__(self, cfg):
        ...
        self.att = MultiHeadAttention(
            d_in=cfg["emb_dim"],
            d_out=cfg["emb_dim"],
            context_length=cfg["text_max_len"],
...
# ---------------------------------------------------------------------------
# Decoder Block  (causal self-attention + cross-attention to text context)
# ---------------------------------------------------------------------------
...
class DecoderBlock(nn.Module):
    def __init__(self, cfg):
        ...
        # Step 2 — cross-attention: image tokens (Q) attend to text context (K, V)
...
class TextEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.pos_emb = nn.Embedding(cfg["text_max_len"], cfg["emb_dim"])
        self.drop    = nn.Dropout(cfg["drop_rate"])
    ...
    def forward(self, token_ids):
        ...
        x = self.tok_emb(token_ids) + self.pos_emb(
            torch.arange(seq_len, device=token_ids.device)
        )
        x = self.drop(x)
        x = self.blocks(x)
        return self.norm(x)   # (B, text_len, emb_dim)
...
class CraftViT(nn.Module):
    ...
    def forward(self, text_ids, pixel_ids):
        context = self.text_encoder(text_ids)          # (B, text_len, emb_dim)
    ...
    def generate(self, text_ids, device, temperature=1.0):
        ...
        cfg     = self.cfg
        bos     = cfg["bos_token"]
        context = self.text_encoder(text_ids.to(device))
...
if __name__ == "__main__":
    ...
    print(f"CraftViT parameters: {total_params:,}")
